...
from models import AuditLog, Cuti, LoginLog, Notification, User, db
...
def log_login_attempt(user_id, username, success):
    ...
    login_log = LoginLog(user_id=user_id, username=username, ip_address=request.remote_addr, success=success)
        ...
        events.append(
            {
                ...
                "color": color,
                "extendedProps": {"nip": cuti.nip, "jabatan": cuti.jabatan, "status": cuti.status},
            }
        )
        ...
        events.append(
            {
                ...
                "color": "#007bff",
                "extendedProps": {"status": "Google Calendar", "keterangan": google_event.get("description", "")},
            }
        )
...
def inject_system_info():
    return {"system_name": app.config["SYSTEM_NAME"], "organization": app.config["ORGANIZATION"], "now": datetime.now()}
...
def reset_password_token(token):
    try:
        ...
        email = serializer.loads(
            token, salt=app.config["SECURITY_PASSWORD_SALT"], max_age=3600  # Token expired after 1 hour
        )
...
def dashboard():
    ...
    # SQLAlchemy
    cuti_disetujui = (
        db.session.query(db.func.sum(Cuti.jumlah_hari))
    ...
    cuti_terakhir = (
        Cuti.query.filter(Cuti.user_id == current_user.id).order_by(Cuti.tanggal_mulai.desc()).limit(5).all()
    )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    send_email(to=current_user.email, subject="Pengajuan Cuti Berhasil Dikirim", html=html_content)
                except Exception as e:
...
def status_cuti():
    ...
    if current_user.role == "admin":
        ...
        cuti_list = [{"cuti": cuti, "username": user.username, "nip": user.nip} for cuti, user in results.items]
        ...
        cuti_list = [
            {"cuti": cuti, "username": current_user.username, "nip": current_user.nip} for cuti in results.items
        ]

    total_pages = results.pages

    return render_template(
        "user/status_cuti.html", cuti_list=cuti_list, page=page, total_pages=total_pages, per_page=per_page
    )
...
def cetak_surat(cuti_id):
    ...
    if current_user.role == "admin":
        ...
        cuti = db.session.query(Cuti).join(User).filter(Cuti.id == cuti_id, Cuti.user_id == current_user.id).first()
    ...
    try:
        ...
        try:
            pdf = pdfkit.from_string(
                ...
                configuration=pdfkit.configuration(wkhtmltopdf=app.config.get("WKHTMLTOPDF_PATH")),
            )
        except OSError as e:
        ...
        response.headers["Content-Disposition"] = f"attachment; filename=surat_cuti_{cuti.user.nip}_{cuti.id}.pdf"
        return response
        ...
        events.append(
            {
                ...
                "extendedProps": {"status": c.status, "keterangan": c.perihal_cuti},
            }
        )
...
def admin_dashboard():
    try:
        ...
        rekap_per_bulan = (
            db.session.query(db.func.strftime("%Y-%m", Cuti.created_at).label("bulan"), db.func.count().label("jumlah"))
            .group_by("bulan")
            .order_by(text("bulan desc"))
            .limit(6)
            .all()
        )
        ...
        return render_template("error.html", error_message="Terjadi kesalahan database. Silakan coba lagi nanti."), 500

    except Exception:
        current_app.logger.error("Unexpected error", exc_info=True)
        return render_template("error.html", error_message="Terjadi kesalahan sistem. Tim kami telah diberitahu."), 500
...
def manage_users():
    ...
    users = query.order_by(User.username.asc()).paginate(page=page, per_page=per_page, error_out=False).items

    return render_template("admin/manage_users.html", users=users, search=search, page=page, total_pages=total_pages)
...
def atasan_dashboard():
    ...
    total_pending = Cuti.query.join(User).filter(User.atasan_id == current_user_id, Cuti.status == "Pending").count()

    total_approved = Cuti.query.join(User).filter(User.atasan_id == current_user_id, Cuti.status == "Approved").count()

    total_rejected = Cuti.query.join(User).filter(User.atasan_id == current_user_id, Cuti.status == "Rejected").count()
...
def manage_cuti():
    try:
        ...
        # foreign key
        query = (
            db.session.query(Cuti)
            .join(User, Cuti.user_id == User.id)  # Explicit join condition
            .filter(
        ...
        if search:
            query = query.filter(
                (User.nip.ilike(f"%{search}%")) | (User.username.ilike(f"%{search}%"))  # Case-insensitive search
            )
        ...
        return render_template("error.html", error_message="Terjadi kesalahan saat memuat data cuti"), 500
...
def handle_unexpected_error(e):
    ...
    return (
        render_template(
            "errors/500.html", error=e, system_name=app.config["SYSTEM_NAME"], organization=app.config["ORGANIZATION"]
        ),
        500,
    )
